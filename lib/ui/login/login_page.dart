...
class _LoginState extends State<LoginPage> {
  ...
  final DatabaseProvider _dataBaseService = DatabaseProvider();
  ...
  void handleLogin() async {
    ...
    if (form.validate()) {
      ...
      List resultData = await db.cekData(_email,_password);
      log(resultData.toString());
      if(resultData.isEmpty){
        scaffoldKey.currentState.showSnackBar(new SnackBar(content: new Text("Login gagal , periksa kembali inputan anda")));
        // var suggestion = Employee(name, nik, bytes);
        // result = await db.saveEmployee("employee", suggestion);
      } else {
        Navigator.push(
          context,
          MaterialPageRoute(
